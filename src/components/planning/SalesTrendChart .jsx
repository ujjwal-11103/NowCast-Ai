...
const SalesTrendChart = ({ chartToggle = { oos: false, seasonalityTrends: false }, customTitle, hideLegend = false, manualConsensus = null }) => {
    ...
    const {
        ...
        // NEW LOGIC: Only show Consensus Line if there is a meaningful difference vs Baseline
        const isConsensusDifferent = consensusData.some((val, idx) => {
            // If consensus value is missing (e.g. History period where we don't calculate consensus), ignore.
            if (val === null) return false;

            const baseline = forecastsData[idx];

            // Compare values (tolerance of 1 to avoid float dust)
            return Math.abs(val - (baseline || 0)) > 1;
        });
        ...
        return {
            ...
            showConsensusLine: isConsensusDifferent,
            showWhatIfLine
        };
