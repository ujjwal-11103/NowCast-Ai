...
const SalesTrendChart = ({ chartToggle = { oos: false, seasonalityTrends: false } }) => {
    ...
    const {
        ...
        if (!globalData) return { chartDates: [], chartActuals: [], chartForecasts: [], chartConsensus: [], chartWhatIf: [], chartOOS: [], chartSeasonality: [], chartTrends: [], showConsensusLine: false, showWhatIfLine: false };
    ...
    }, [filters, globalData, whatIfData, chartToggle]);
    ...
    const [chartRevision, setChartRevision] = React.useState(0);

    React.useEffect(() => {
        if (!chartContainerRef.current) return;

        const resizeObserver = new ResizeObserver(() => {
            // Force a re-render/resize by incrementing revision or triggering a window resize event
            // Triggering window resize is the most reliable way for react-plotly's useResizeHandler
    ...
    return (
        <div ref={chartContainerRef} className="w-full h-full">
            <Plot
                data={chartTraces}
                layout={{ ...layoutConfig, autosize: true }}
